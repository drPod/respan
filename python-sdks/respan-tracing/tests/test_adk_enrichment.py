...
class TestEnrichedSpan:
    ...
    def test_promote_to_root(self):
        """When promote_to_root=True, parent is None."""
        original = _make_adk_span("span", {}, parent_span_id=0x1234)
        enriched = EnrichedSpan(original, promote_to_root=True)
        assert enriched.parent is None
        assert enriched._parent is None
        assert enriched.parent_span_id is None

    def test_promote_to_root_false(self):
        """When promote_to_root=False, parent is forwarded."""
        original = _make_adk_span("span", {}, parent_span_id=0x1234)
        enriched = EnrichedSpan(original, promote_to_root=False)
        assert enriched.parent is not None
