...
class TestEnrichAdkSpans:
    ...
    def test_mixed_adk_and_non_adk(self):
        ...
        assert len(result) == 2
        # Non-ADK span returned unchanged
        assert non_adk in result
        # ADK span should be enriched (wrapped in EnrichedSpan)
    ...
    def test_adk_spans_grouped_by_trace_id(self):
        ...
        assert len(result) == 2
        # Both should be enriched (each is an invocation in its own trace group)
        for span in result:
...
class TestEnrichAdkTraceGroup:
    ...
    def test_root_invocation_renamed_to_agent_name(self):
        ...
        invocation_span = enriched[0]
        assert invocation_span.name == "MyAgent"
    ...
    def test_root_invocation_gets_workflow_kind(self):
        ...
        enriched = _enrich_adk_trace_group(spans)
        invocation_span = enriched[0]
        assert invocation_span.attributes["traceloop.span.kind"] == "workflow"
    ...
    def test_input_output_propagated_to_root(self):
        ...
        enriched = _enrich_adk_trace_group(spans)
        invocation_span = enriched[0]
        assert invocation_span.attributes["traceloop.entity.input"] == '[{"role": "user", "content": "hi"}]'
        assert invocation_span.attributes["traceloop.entity.output"] == '{"role": "assistant", "content": "hello"}'

    def test_child_spans_get_entity_path(self):
        """Non-invocation spans get traceloop.entity.path = 'adk.<prefix>'."""
        spans = _make_standard_trace_group()
        enriched = _enrich_adk_trace_group(spans)
        # invoke_agent span (index 1)
        assert enriched[1].attributes["traceloop.entity.path"] == "adk.invoke_agent"
        # call_llm span (index 2)
    ...
    def test_llm_span_gets_request_type(self):
        ...
        enriched = _enrich_adk_trace_group(spans)
        llm_span = enriched[2]
        assert llm_span.attributes["llm.request.type"] == "chat"
    ...
    def test_attribute_stripping(self):
        ...
        # Build a call_llm span with several strip-target attributes
        trace_id = 0xABCD
        call_llm = _make_adk_span(
            "call_llm",
            {
        ...
        # Verify at least these four are stripped
    ...
    def test_session_id_as_thread_identifier(self):
        ...
        spans = _make_standard_trace_group(
            ...
            resource_attrs={},  # no respan.threads.thread_identifier
        )
        enriched = _enrich_adk_trace_group(spans)
        invocation_span = enriched[0]
        assert invocation_span.attributes["respan.threads.thread_identifier"] == "session-xyz"

    def test_customer_identifier_propagated(self):
        """Resource-level customer_identifier is propagated to the root span."""
        spans = _make_standard_trace_group(
            resource_attrs={
                "respan.customer_params.customer_identifier": "cust-42",
            },
        )
        enriched = _enrich_adk_trace_group(spans)
        invocation_span = enriched[0]
        assert invocation_span.attributes["respan.customer_params.customer_identifier"] == "cust-42"
    ...
    def test_llm_span_input_output_from_adk_fallback(self):
        ...
        invoke_agent = _make_adk_span(
            "invoke_agent",
            {"gen_ai.agent.name": "FallbackAgent"},
            trace_id=trace_id,
            span_id=0x0002,
            parent_span_id=0x0001,
        )
        # call_llm span with only ADK-native attributes (no gen_ai.input.messages)
        ...
        # The call_llm span should have input/output extracted via fallback
        ...
        # Root invocation should also have propagated I/O
        root_enriched = enriched[0]
        root_input = json.loads(root_enriched.attributes["traceloop.entity.input"])
    ...
    def test_resource_thread_id_takes_precedence(self):
        ...
        spans = _make_standard_trace_group(
            ...
            resource_attrs={
                "respan.threads.thread_identifier": "explicit-thread-id",
            },
        )
        enriched = _enrich_adk_trace_group(spans)
        invocation_span = enriched[0]
        assert invocation_span.attributes["respan.threads.thread_identifier"] == "explicit-thread-id"
    ...
    def test_invocation_without_agent_name_keeps_original_name(self):
        ...
        # invoke_agent without gen_ai.agent.name
        invoke_agent = _make_adk_span(
        ...
        # name should stay as original "invocation" (EnrichedSpan with name=None)
