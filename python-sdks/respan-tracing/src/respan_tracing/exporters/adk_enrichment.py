"""ADK span enrichment for the OTLP exporter.

Enriches Google ADK spans with OpenLLMetry-compatible attributes so the
backend can extract trace-level fields (name, input, output, tokens).

This module is internal — it is called by ``RespanSpanExporter.export()``
and should not be imported by end users.
"""
